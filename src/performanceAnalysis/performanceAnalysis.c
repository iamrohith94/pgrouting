...
#include "drivers/performanceAnalysis/performanceAnalysis_driver.h"  // the link to the C++ code of the function

PGDLLEXPORT Datum performanceAnalysis(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(performanceAnalysis);
...
process(
        ...
        bool directed,
        bool only_cost,
        Performance_analysis_t **result_tuples,
        size_t *result_count) {
    ...
    edge_read_time = (double)(1000.0 * (end_t-start_t) / CLOCKS_PER_SEC);;

    PGR_DBG("Total %ld edges in query:", total_edges);
    // PGR_DBG("Time taken to read edges: %f", edge_read_time);
    if (total_edges == 0) {
    ...
    // Populating the edge read time
    for (size_t i = 0; i < *result_count; i++) {
        ((*result_tuples) + i)->edge_read_time = edge_read_time;
    }
...
PGDLLEXPORT Datum performanceAnalysis(PG_FUNCTION_ARGS) {
    ...
    /*                          MODIFY AS NEEDED                              */
    /*                                                                        */
    Performance_analysis_t  *result_tuples = NULL;
    ...
    result_tuples = (Performance_analysis_t*) funcctx->user_fctx;
    ...
    OUT time_taken FLOAT
        ...
        values[5] = Float8GetDatum(result_tuples[funcctx->call_cntr].edge_read_time);
