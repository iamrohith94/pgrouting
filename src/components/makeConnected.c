...
#include "drivers/components/makeConnected_driver.h"  // the link to the C++ code of the function

PGDLLEXPORT Datum makeConnected(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(makeConnected);
...
process(
        char* edges_sql,
        bool directed,
        pgr_connections_rt **result_tuples,
        size_t *result_count) {
    /*
...
PGDLLEXPORT Datum makeConnected(PG_FUNCTION_ARGS) {
    ...
    /*                          MODIFY AS NEEDED                              */
    /*                                                                        */
    pgr_connections_rt *result_tuples = NULL;
    ...
    result_tuples = (pgr_connections_rt*) funcctx->user_fctx;
