...
``pgr_timeAnalysis`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: timeAnalysis(Minimal Use)
...
.. code-block:: none
    ...
    pgr_timeAnalysis(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_timeAnalysis.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: timeAnalysis(Complete signature)
...
.. code-block:: none
    ...
    pgr_timeAnalysis(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
