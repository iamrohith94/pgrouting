...
``pgr_betweenness`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: betweenness(Minimal Use)
...
.. code-block:: none
    ...
    pgr_betweenness(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_betweenness.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: betweenness(Complete signature)
...
.. code-block:: none
    ...
    pgr_betweenness(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
